...
is some highly illegal contraband. This creates a Variable-Length Array (VLA), not a constant-sized array with size `5`. What's worse is that the Usual Compilers™ (GCC, Clang, ICC, MSVC, and most other optimizing compilers actually worth compiling with) were typically powerful enough to basically turn the code generation of this object -- so long as you didn't pass it to something expecting an actual [Variably-Modified Types (also talked about in another post)](/ever-closer-c23-improvements#separating-variably-modified-types-from-variable-length-arrays) -- into working like a normal C array.
