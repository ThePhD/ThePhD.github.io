...
- what-you-write-is-what's-inside (if you write a function `foo`, you get pretty directly a function named `foo`);
- uncompromised, direct access to the hardware (through close collaboration with implementation-defined `asm`, intrinsics, and unparalleled control of your compiler);
...
- and, being a smaller language focused on its use cases (K&R literally sold C on being good at strings, and you can see how that's been going in the last 30 years).
...
This is not even "ugh, C should just be C++". It is moreso that if feels like a lot of people are really trying to invent these things from first principles and pretend like they were the first people to ever conceive of these ideas, evaluating effectively no prior art in other languages, domains, C codebases themselves, just... anything. It's a constant feeling when working on things like C that for the life of me I cannot seem to shake no matter how carefully I break it down. Hell, part of writing this post is so I can stick a link to it in my `defer` paper and in the `defer` Technical Specification when it happens so I don't have to sit down and walk through why I chose a procedural-style, object-less idiom for C rather than trying to load the RAII shotgun and point it at our beloved 746-and-counting page C standard.
